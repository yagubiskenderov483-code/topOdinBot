...
async def cmd_neptune(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(text,parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[back_btn(lang,"main_menu")]]))
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if d=="req_del_menu":
            ...
            rows.append([back_btn(lang,"menu_req")])
        ...
        if d.startswith("req_edit_"):
            ...
            if raw.endswith("_buyer"):
                ...
                await send_section(update,prompts.get(field,"?"),
                    InlineKeyboardMarkup([[back_btn(lang,"menu_deal")]]),section="profile"); return
            ...
            ud["req_step"]=field
            await send_section(update,prompts.get(field,"?"),
                InlineKeyboardMarkup([[back_btn(lang,"menu_req")]]),section="profile"); return
        ...
        if d.startswith("req_deal_"):
            ...
            await send_section(update,prompts.get(field,"?"),
                InlineKeyboardMarkup([[back_btn(lang,f"add_req_{deal_id}")]]),section="deal_card"); return
        ...
        if d.startswith("withdraw_"):
            ...
            await send_section(update,
                ...
                InlineKeyboardMarkup([[back_btn(lang,"withdraw")]]),section="balance"); return
            ...
            if field=="card":
                if ud.get("card_step")=="bank":
                    ...
                    if not text.strip() or not _re3.match(r"^[a-zA-Zа-яёА-ЯЁ ]+$", text.strip()):
                        await update.message.reply_text(f"{Ewrn} <b>{R(ru,'Введите название банка (только буквы, рус или англ):','Enter bank name (letters only, ru or en):')}</b>",parse_mode="HTML"); return
    ...
    try:
        ...
        if s:
            try:
                ...
                await context.bot.send_message(chat_id=int(s),
                    text=R(rs,f"{Ech} <b>Сделка завершена!</b>",f"{Ech} <b>Deal completed!</b>"),parse_mode="HTML")
            except: pass
        ...
        if buyer_uid:
            try:
                ...
                if creator_role=="seller":
                    msg_buyer=R(rb2,f"{Ech} <b>Оплата подтверждена!</b>",f"{Ech} <b>Payment confirmed!</b>")
                else:
                    msg_buyer=R(rb2,f"{Ech} <b>Сделка подтверждена!</b>",f"{Ech} <b>Deal confirmed!</b>")
    ...
    try:
        ...
        if not deals:
            await send_section(update,
                ...
                InlineKeyboardMarkup([[back_btn(lang,"main_menu")]]),section="my_deals"); return
        SNAMES={
        ...
        await send_section(update,"\n".join(lines),
            InlineKeyboardMarkup([[back_btn(lang,"main_menu")]]),section="my_deals")
...
async def show_top(update, context):
    try:
        ...
        await send_section(update,"\n".join(lines),
            InlineKeyboardMarkup([[back_btn(lang,"main_menu")]]),section="top")
    ...
    try:
        ...
        if bal<=0:
            await send_section(update,
                ...
                InlineKeyboardMarkup([[back_btn(lang,"menu_balance")]]),section="balance"); return
