...
# Названия типов сделок (включая premium_stickers)
TNAMES = {
    "nft":              "🖼 NFT",
    "username":         "👤 Юзернейм",
    "stars":            "⭐️ Звёзды",
...
def cur_name(code, lang):
    ...
    if isinstance(v, dict):
        return v.get(lang, v.get("ru", code))
    return v
...
def main_kb(lang):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(b["balance"],     callback_data="menu_balance"),
         InlineKeyboardButton("📋 Мои сделки", callback_data="menu_my_deals")],
...
async def show_main(update, context, edit=False):
    try:
        ...
        logger.error(f"show_main error: {e}")
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # ── НОВЫЙ: Премиум стикеры ───────────────────────────────────────────
        if d == "dt_pst":
            ud.clear(); ud["type"] = "premium_stickers"; ud["step"] = "partner"
        ...
        if d == "cry_ton":
            ...
            await send_text(update, "💎 <b>Крипта (TON)\n\nВведите сумму сделки:</b>")
            return
        if d == "cry_usd":
            ud["currency"] = "USDT"; ud["step"] = "amount"
            await send_text(update, "💎 <b>Крипта (USDT)\n\nВведите сумму сделки:</b>")
            return
        ...
        # ── НОВЫЙ: Количество стикерпаков ────────────────────────────────────
        if d in ("pst_1", "pst_3", "pst_5", "pst_10"):
            counts = {"pst_1": "1 пак", "pst_3": "3 пака", "pst_5": "5 паков", "pst_10": "10 паков"}
            ud["sticker_count"] = counts[d]
            ud["step"] = "currency"
            await send_text(update, "🎭 <b>Премиум стикеры\n\nВыберите валюту:</b>", cur_kb(lang))
            return
        ...
        if d.startswith("cur_"):
            ...
            ud["step"] = "amount"
            icons = {"nft": E["nft"], "username": E["user"], "stars": E["star"],
                     "premium": E["premium"], "premium_stickers": E["sticker"]}
            icon = icons.get(ud.get("type", ""), E["deal"])
            await send_text(update, f"{icon} <b>Введите сумму сделки:</b>")
            return

    except Exception as e:
        logger.error(f"on_cb error: {e}")
...
async def on_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if uid == ADMIN_ID and ud.get("adm_step") == "banner":
            await handle_adm_msg(update, context); return
        ...
        if step == "partner":
            if not text.startswith("@"):
                await update.message.reply_text(
                    f"{E['cross']} <b>Юзернейм должен начинаться с @</b>", parse_mode="HTML")
                return
            ud["partner"] = text
            if dtype == "nft":
                ud["step"] = "nft_link"
            ...
            # ── НОВЫЙ: шаг для премиум стикеров ─────────────────────────────
                ...
                await update.message.reply_text(
                    ...
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("1 пак",    callback_data="pst_1"),
                         InlineKeyboardButton("3 пака",   callback_data="pst_3")],
                        [InlineKeyboardButton("5 паков",  callback_data="pst_5"),
                         InlineKeyboardButton("10 паков", callback_data="pst_10")],
                    ]))
            return
        ...
        if step == "nft_link":
            if not text.startswith("https://"):
                await update.message.reply_text(
                    f"{E['cross']} <b>Ссылка должна начинаться с https://</b>", parse_mode="HTML")
                return
            ud["nft_link"] = text
            ud["step"] = "currency"
            await update.message.reply_text(
        ...
        if step == "stars_cnt":
            if not text.isdigit():
                await update.message.reply_text(
                    f"{E['cross']} <b>Только цифры!</b>", parse_mode="HTML")
                return
            ud["stars_count"] = text
            ud["step"] = "currency"
            await update.message.reply_text(
...
async def finalize_deal(update, context):
    try:
        ...
        if pname:
            puid = next((k for k, v in db["users"].items()
                         if v.get("username", "").lower() == pname), None)
            if puid:
                try:
                    txt = build_buyer_card(deal_id, db["deals"][deal_id],
                                           f"@{user.username or str(user.id)}")
                    kb  = InlineKeyboardMarkup([
                        [InlineKeyboardButton("✅ Я оплатил",   callback_data=f"paid_{deal_id}")],
                        [InlineKeyboardButton("🏠 Главное меню", callback_data="main_menu")],
                    ])
                    await context.bot.send_message(
                        chat_id=int(puid), text=txt, parse_mode="HTML", reply_markup=kb)
                except Exception as e:
                    logger.error(f"notify partner error: {e}")
        context.user_data.clear()
    except Exception as e:
        logger.error(f"finalize_deal error: {e}")
...
async def send_deal_card(update, context, deal_id, d, buyer=False):
    try:
        ...
        pay     = (f"\n━━━━━━━━━━━━━━━━━━━━\n"
                   ...
                   f"✅ После перевода нажмите «Я оплатил»")
        if buyer:
            pu   = (f"https://t.me/{partner.lstrip('@')}" if partner.startswith("@")
                    else "https://t.me/GiftDealsManager")
        ...
        logger.error(f"send_deal_card error: {e}")
...
async def on_paid(update, context):
    try:
        ...
        adm_kb_inline = InlineKeyboardMarkup([[
        ...
        try:
            await context.bot.send_message(
                chat_id=ADMIN_ID, text=adm_txt, parse_mode="HTML", reply_markup=adm_kb_inline)
        except Exception as e:
            logger.error(f"on_paid admin notify error: {e}")
        ...
        if seller and seller != str(buyer.id):
            try:
                ...
                logger.error(f"on_paid seller notify error: {e}")
        try:
            ...
            logger.error(f"on_paid edit markup error: {e}")
    except Exception as e:
        logger.error(f"on_paid error: {e}")
    ...
    try:
        ...
        if deal_id in db.get("deals", {}):
            ...
            try:
                ...
                logger.error(f"adm_confirm edit error: {e}")
            if s:
                try:
                    await context.bot.send_message(
                        chat_id=int(s),
                    ...
                    logger.error(f"adm_confirm seller notify error: {e}")
    except Exception as e:
        logger.error(f"adm_confirm error: {e}")
...
async def adm_decline(update, context):
    try:
        ...
        try:
            ...
            logger.error(f"adm_decline edit error: {e}")
    except Exception as e:
        logger.error(f"adm_decline error: {e}")
...
async def show_balance(update, context):
    try:
        ...
        logger.error(f"show_balance error: {e}")
...
async def show_balance_info(update, context, method):
    try:
        ...
        await edit_or_send(update, text,
            InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="menu_balance")]]))
    except Exception as e:
        logger.error(f"show_balance_info error: {e}")
...
async def show_lang(update, context):
    try:
        ...
        logger.error(f"show_lang error: {e}")
...
async def show_profile(update, context):
    try:
        ...
        logger.error(f"show_profile error: {e}")
...
async def show_my_deals(update, context):
    try:
        ...
        lines  = [E["deal"] + f" <b>Мои сделки ({len(deals)}):</b>"]
        ...
        logger.error(f"show_my_deals error: {e}")
...
async def show_top(update, context):
    try:
        ...
        logger.error(f"show_top error: {e}")
...
async def show_withdraw(update, context):
    try:
        ...
        try:
            ...
            logger.error(f"show_withdraw admin notify error: {e}")
        await edit_or_send(update,
        ...
        logger.error(f"show_withdraw error: {e}")
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"{E['shield']} <b>Панель администратора</b>", parse_mode="HTML", reply_markup=adm_kb())
...
async def handle_adm_cb(update, context):
    try:
        ...
        if d == "adm_deals":
            ...
            for did, dv in list(deals.items())[-10:]:
                text += (f"\n<b>{did}</b> | {TNAMES.get(dv.get('type',''), dv.get('type',''))} "
                         f"| {dv.get('amount')} {dv.get('currency')} | {dv.get('status')}")
            await edit_or_send(update, text,
                InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="adm_back")]]))
            return
        action_map = {
        ...
        logger.error(f"handle_adm_cb error: {e}")
...
async def handle_adm_msg(update, context):
    try:
        ...
        if step == "get_user":
            ...
            found = next((k for k, v in db["users"].items()
                          if v.get("username", "").lower() == uname), None)
            if not found:
            ...
            u = db["users"][found]
            await update.message.reply_text(
                f"<b>@{u.get('username','—')} | Сделок: {u.get('total_deals',0)} "
                f"| Реп: {u.get('reputation',0)}\nСтатус: {u.get('status','—')}</b>",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([
        ...
        if step == "banner":
            if update.message and update.message.photo:
                ...
                save_db(db)
                await update.message.reply_text(
                    f"{E['check']} <b>Фото-баннер установлен!</b>", parse_mode="HTML", reply_markup=ok_kb)
                ...
                save_db(db)
                await update.message.reply_text(
                    f"{E['check']} <b>Видео-баннер установлен!</b>", parse_mode="HTML", reply_markup=ok_kb)
            elif text.lower() == "off":
                db["banner"] = db["banner_photo"] = db["banner_video"] = None
                save_db(db)
                await update.message.reply_text(
                    f"{E['check']} <b>Баннер удалён!</b>", parse_mode="HTML", reply_markup=ok_kb)
            else:
                db["banner"] = text
                db["banner_photo"] = db["banner_video"] = None
                save_db(db)
                await update.message.reply_text(
                    f"{E['check']} <b>Баннер установлен!</b>", parse_mode="HTML", reply_markup=ok_kb)
            ud["adm_step"] = None
            return
        ...
        if step == "menu_desc":
            ...
            save_db(db)
            await update.message.reply_text(
                f"{E['check']} <b>Описание обновлено!</b>", parse_mode="HTML", reply_markup=ok_kb)
            ud["adm_step"] = None
            return
        ...
        if step == "set_value":
            ...
            save_db(db)
            await update.message.reply_text(
                f"{E['check']} <b>Обновлено!</b>", parse_mode="HTML", reply_markup=ok_kb)
            ud["adm_step"] = None
            return
    except Exception as e:
        logger.error(f"handle_adm_msg error: {e}")

# ── Secret / utility commands ─────────────────────────────────────────────────
async def cmd_neptune(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        await update.message.reply_text(
            ...
            "🔹 /set_my_amount [сумма]\n"
            "   Пример: /set_my_amount 50000</b>",
            parse_mode="HTML")
    except Exception as e:
        logger.error(f"cmd_neptune error: {e}")
...
async def cmd_buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if deal_id not in db.get("deals", {}):
            await update.message.reply_text("<b>Не найдено.</b>", parse_mode="HTML")
            return
        db["deals"][deal_id]["status"] = "confirmed"
        s = db["deals"][deal_id].get("user_id")
        if s and s in db["users"]:
        ...
        save_db(db)
        await update.message.reply_text(
            f"{E['check']} <b>Сделка {deal_id} подтверждена!</b>", parse_mode="HTML")
    except Exception as e:
...
async def cmd_set_deals(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.error(f"cmd_set_deals error: {e}")
...
async def cmd_set_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.error(f"cmd_set_amount error: {e}")
