            ...
            if d:
                ...
                if not any(buyer_reqs.get(f) for f in ("card","ton","stars")):
                    kb=InlineKeyboardMarkup([
                        ...
                        [InlineKeyboardButton(f"{ce('5264713049637409446','💎')} TON",callback_data=f"req_deal_ton_{deal_id}")],
                        [InlineKeyboardButton(f"{ce('5267500801240092311','⭐')} {R(ru,'Звёзды','Stars')}",callback_data=f"req_deal_stars_{deal_id}")],
                    ])
                    no_req_text = R(
                        ru,
                ...
                kb_buyer=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton(f"{ce('5287231198098117669','🤝')} {R(ru,'Написать продавцу','Write to seller')}",url=pu)],
                ...
                if seller_uid:
                    try:
                        ...
                        seller_kb=InlineKeyboardMarkup([
                            [InlineKeyboardButton(f"{ce('5287231198098117669','🤝')} {R(rs,'Написать покупателю','Write to buyer')}",url=burl)],
        ...
        if d in ("role_buyer","role_seller"):
            ...
            if not any(reqs.get(f) for f in ("card","ton","stars")):
                ...
                kb=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton(f"{ce('5264713049637409446','💎')} TON",callback_data="req_edit_ton_buyer")],
                    [InlineKeyboardButton(f"{ce('5267500801240092311','⭐')} {R(ru,'Звёзды','Stars')}",callback_data="req_edit_stars_buyer")],
        ...
        if d=="balance_topup":
            await send_section(update,
                ...
                InlineKeyboardMarkup([
                    [InlineKeyboardButton(f"{ce('5267500801240092311','⭐')} {R(ru,'Звёзды','Stars')}",callback_data="topup_cur_stars")],
                    ...
                    [InlineKeyboardButton(f"{ce('5242606681166220600','🤖')} TON - Crypto Bot",callback_data="topup_cur_ton_only")],
                    [InlineKeyboardButton(f"{ce('5397829221605191505','💎')} USDT - Tonkeeper",callback_data="topup_cur_usdt_tonkeeper")],
                    [InlineKeyboardButton(f"{ce('5242606681166220600','🤖')} USDT - Crypto Bot",callback_data="topup_cur_usdt_only")],
            ...
            if not any(reqs.get(f) for f in ("card","ton","stars")):
                await send_section(update,
                    ...
                    InlineKeyboardMarkup([
                        ...
                        [InlineKeyboardButton(f"{ce('5264713049637409446','💎')} {R(ru,'Добавить TON','Add TON')}",callback_data="req_edit_ton")],
                        [InlineKeyboardButton(f"{ce('5267500801240092311','⭐')} {R(ru,'Добавить @username','Add @username')}",callback_data="req_edit_stars")],
                        [InlineKeyboardButton(f"{ce('5877629862306385808','◀️')} {R(ru,'Назад','Back')}",callback_data="menu_balance")],
                    ]),section="balance"); return
    ...
    try:
        ...
        if ud.get("withdraw_step")=="req":
            ...
            await update.message.reply_text(
                ...
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton(f"{ce('5287231198098117669','🤝')} {R(ru,'Менеджер','Manager')}",url=MANAGER_URL)],
                    [InlineKeyboardButton(f"{ce('5316887736823591263','🏠')} {R(ru,'Главное меню','Main menu')}",callback_data="main_menu")],
                ])); return
        ...
        if step=="partner":
            ...
            if dtype=="nft":
                ...
                await send_step(
                    ...
                    InlineKeyboardMarkup([
                        [InlineKeyboardButton(f"{ce('5264713049637409446','💎')} TON",callback_data="cry_ton"),
                ...
                await send_step(f"{Eprem} <b>Telegram Premium\n\n{R(ru,'Выберите период:','Choose period:')}</b>",
                    InlineKeyboardMarkup([[
                        InlineKeyboardButton(f"{ce('5370784581341422520','⭐')} 3 {R(ru,'мес.','mo')}",callback_data="prm_3"),
                        InlineKeyboardButton(f"{ce('5370784581341422520','⭐')} 6 {R(ru,'мес.','mo')}",callback_data="prm_6"),
                        InlineKeyboardButton(f"{ce('5370784581341422520','⭐')} 12 {R(ru,'мес.','mo')}",callback_data="prm_12")]]))
            return
...
async def finalize_deal(update, context):
    try:
        ...
        text+=f"🔗 <b>{ll}:</b>\n<code>{join_link}</code>"
        kb=InlineKeyboardMarkup([
            [InlineKeyboardButton(f"{ce('5877465816030515018','🔥')} {lbl_send}",switch_inline_query=f"Заходи в сделку\n{join_link}")],
            [InlineKeyboardButton(f"{ce('5316887736823591263','🏠')} {R(ru,'Главное меню','Main menu')}",callback_data="main_menu")]
        ])
            ...
            if puid:
                try:
                    ...
                    kb2=InlineKeyboardMarkup([
                        [InlineKeyboardButton(f"{ce('5902335789798265487','🤝')} {R(pr,'Присоединиться','Join')}",url=join_link)],
...
async def show_lang(update, context):
    try:
        ...
        rows=[
            [InlineKeyboardButton(f"{ce('5258115571848846212','🌍')} Русский",callback_data="lang_ru")],
            [InlineKeyboardButton(f"{ce('5258115571848846212','🌍')} English",callback_data="lang_en")],
        ]
...
async def show_req(update, context):
    try:
        ...
        if ton:
            ...
            rows.append([InlineKeyboardButton(f"{ce('5264713049637409446','💎')} {R(ru,'Добавить TON','Add TON')}",callback_data="req_edit_ton")])
        if stars:
            ...
            rows.append([InlineKeyboardButton(f"{ce('5267500801240092311','⭐')} {R(ru,'Добавить Звёзды','Add Stars')}",callback_data="req_edit_stars")])
        rows.append([InlineKeyboardButton(f"{ce('5877629862306385808','◀️')} {R(ru,'Назад','Back')}",callback_data="main_menu")])
...
async def show_withdraw(update, context):
    try:
        ...
        rows=[]
        if reqs.get("ton"): rows.append([InlineKeyboardButton(f"{ce('5264713049637409446','💎')} TON/USDT → "+reqs["ton"][:12]+"...",callback_data="withdraw_crypto")])
        else: rows.append([InlineKeyboardButton(f"{ce('5264713049637409446','💎')} TON / USDT",callback_data="withdraw_crypto")])
        if reqs.get("stars"): rows.append([InlineKeyboardButton(f"{ce('5267500801240092311','⭐')} "+R(ru,"Звёзды → ","Stars → ")+reqs["stars"],callback_data="withdraw_stars")])
        else: rows.append([InlineKeyboardButton(f"{ce('5267500801240092311','⭐')} {R(ru,'Звёзды','Stars')}",callback_data="withdraw_stars")])
    ...
    try:
        ...
        if d=="adm_menu_desc":
            ...
            await q.message.edit_text("<b>Введите новое описание меню:</b>",parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{ce('5877629862306385808','◀️')} Отмена",callback_data="adm_back")]])); return
...
async def handle_adm_msg(update, context):
    try:
        ...
        db=load_db(); ok_kb=InlineKeyboardMarkup([[InlineKeyboardButton(f"{ce('5316887736823591263','🏠')} Панель",callback_data="adm_back")]])
