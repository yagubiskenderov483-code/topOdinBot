...
def build_deal_text(deal_id, d, creator_tag, partner_tag, lang, joined=False):
    try:
        ...
        pay_cur=cur
        pay_text=""
        if pay_cur=="TON" or pay_cur=="USDT":
            pay_text=(f"\n\n<b>{Eton} {R(ru,'Оплата TON/USDT','Pay TON/USDT')}:</b>\n"
                      f"<blockquote>{R(ru,'Переведите средства через CryptoBot','Send funds via CryptoBot')}\n"
                      f"{CRYPTO_BOT}</blockquote>")
        elif pay_cur=="RUB":
            pay_text=(f"\n\n<b>{Ecrd} {R(ru,'Оплата по карте','Pay by card')}:</b>\n"
                      f"<blockquote><code>{CARD_NUM}</code> ({card_bank(lang)})\n"
                      f"{R(ru,'Получатель','Recipient')}: {CARD_NAME}</blockquote>")
        elif pay_cur=="Stars":
            pay_text=f"\n\n<b>{Est} {R(ru,'Оплата Звёздами','Pay with Stars')}:</b>\n<blockquote>{R(ru,'Используйте встроенную функцию Telegram','Use built-in Telegram feature')}</blockquote>"
        else:
            pay_text=f"\n\n<b>{Ebnk2} {R(ru,'Реквизиты для оплаты','Payment details')}:</b>\n<blockquote>{R(ru,'Контакт менеджера','Contact manager')} {MANAGER_TAG}</blockquote>"
...
async def menu_top_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i,(uid_s,usr) in enumerate(users,1):
        ...
        lines.append(f"{medal}<b>{i}.</b> @{uname} — {deals} {R(ru,'сделок','deals')}, {turn} ₽{status_txt}")
...
async def menu_balance_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text=(f"{Ebal} <b>{R(ru,'Баланс','Balance')}</b>: <b>{bal} ₽</b>\n\n"
...
async def deal_input_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not state or state=="req_input": return
...
async def add_balance_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args=context.args
    if len(args)<2:
        await update.message.reply_text("<b>Использование: /add_balance [uid] [сумма]</b>", parse_mode="HTML")
        return
    
    target_uid=args[0]; amount=args[1]
    try:
        amount_int=int(amount)
    except:
        await update.message.reply_text("<b>❌ Сумма должна быть числом</b>", parse_mode="HTML")
        return
    
    db=load_db(); u=get_user(db, target_uid)
    u["balance"]=u.get("balance",0)+amount_int
    save_db(db)
    
    await update.message.reply_text(f"<b>✅ Баланс пополнен!\n\nUID: {target_uid}\n+{amount_int} ₽</b>", parse_mode="HTML")
...
async def take_balance_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args=context.args
    if len(args)<2:
        await update.message.reply_text("<b>Использование: /take_balance [uid] [сумма]</b>", parse_mode="HTML")
        return
    ...
    await update.message.reply_text(f"<b>✅ Баланс списан!\n\nUID: {target_uid}\n-{amount_int} ₽</b>", parse_mode="HTML")
