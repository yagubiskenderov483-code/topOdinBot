            ...
            if d:
                ...
                if not any(buyer_reqs.get(f) for f in ("card","ton","stars")):
                    kb=InlineKeyboardMarkup([
                        [InlineKeyboardButton(f"💳 {R(ru,'Карта / Телефон','Card / Phone')}",callback_data=f"req_deal_card_{deal_id}")],
                        [InlineKeyboardButton("💎 TON",callback_data=f"req_deal_ton_{deal_id}")],
                        [InlineKeyboardButton(f"⭐️ {R(ru,'Звёзды','Stars')}",callback_data=f"req_deal_stars_{deal_id}")],
                    ])
                    no_req_text = R(
                        ru,
                ...
                if seller_uid:
                    try:
                        ...
                        seller_kb=InlineKeyboardMarkup([
                            [InlineKeyboardButton(f"✉️ {R(rs,'Написать покупателю','Write to buyer')}",url=f"https://t.me/{buyer_uname}" if buyer_uname else MANAGER_URL)],
                            [InlineKeyboardButton(f"🏠 {R(rs,'Главное меню','Main menu')}",callback_data="main_menu")],
                        ])
                ...
                if creator_role=="seller":
                    kb=InlineKeyboardMarkup([
                        [InlineKeyboardButton(f"✅ {R(ru,'Я оплатил','I paid')}",callback_data=f"paid_{deal_id}")],
                        [InlineKeyboardButton(f"✉️ {R(ru,'Написать продавцу','Write to seller')}",url=pu)],
                        [InlineKeyboardButton(f"🏠 {R(ru,'Главное меню','Main menu')}",callback_data="main_menu")],
                    ])
                else:
                    kb=InlineKeyboardMarkup([
                        [InlineKeyboardButton(f"✉️ {R(ru,'Написать менеджеру','Write to manager')}",url=MANAGER_URL)],
                        [InlineKeyboardButton(f"🏠 {R(ru,'Главное меню','Main menu')}",callback_data="main_menu")],
                    ])
    ...
    try:
        ...
        if d in ("role_buyer","role_seller"):
            ...
            if not any(reqs.get(f) for f in ("card","ton","stars")):
                bank=card_bank(lang)
                kb=InlineKeyboardMarkup([
                    [InlineKeyboardButton(f"💳 {R(ru,f'Карта / Телефон {bank}',f'Card / Phone {bank}')}",callback_data="req_edit_card_buyer")],
                    [InlineKeyboardButton("💎 TON",callback_data="req_edit_ton_buyer")],
                    [InlineKeyboardButton(f"⭐️ {R(ru,'Звёзды','Stars')}",callback_data="req_edit_stars_buyer")],
                    [InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="menu_deal")],
                ])
                no_req_text=R(ru,
        ...
        if d in TYPE_MAP:
            ...
            msg=await update.effective_chat.send_message(
                ...
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="menu_deal")]]))
        ...
        if d=="req_del_menu":
            db=load_db(); u=get_user(db,uid); reqs=u.get("requisites",{})
            rows=[]
            if reqs.get("card"): rows.append([InlineKeyboardButton(f"🗑 {R(ru,'Удалить карту/телефон','Delete card/phone')}",callback_data="req_del_card")])
            if reqs.get("ton"):  rows.append([InlineKeyboardButton(f"🗑 {R(ru,'Удалить TON','Delete TON')}",callback_data="req_del_ton")])
            if reqs.get("stars"):rows.append([InlineKeyboardButton(f"🗑 {R(ru,'Удалить @username','Delete @username')}",callback_data="req_del_stars")])
            rows.append([InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="menu_req")])
        ...
        if d.startswith("req_edit_"):
            ...
            if raw.endswith("_buyer"):
                ...
                await send_section(update,prompts.get(field,"?"),
                    InlineKeyboardMarkup([[InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="menu_deal")]]),section="profile"); return
            ...
            ud["req_step"]=field
            await send_section(update,prompts.get(field,"?"),
                InlineKeyboardMarkup([[InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="menu_req")]]),section="profile"); return

        if d.startswith("add_req_"):
            deal_id=d[8:]; ud["req_for_deal"]=deal_id; bank=card_bank(lang)
            kb=InlineKeyboardMarkup([
                [InlineKeyboardButton(f"💳 {R(ru,f'Карта / Телефон {bank}',f'Card / Phone {bank}')}",callback_data=f"req_deal_card_{deal_id}")],
                [InlineKeyboardButton("💎 TON",callback_data=f"req_deal_ton_{deal_id}")],
                [InlineKeyboardButton(f"⭐️ {R(ru,'Звёзды','Stars')}",callback_data=f"req_deal_stars_{deal_id}")],
                [InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="main_menu")],
            ])
        ...
        if d.startswith("req_deal_"):
            ...
            await send_section(update,prompts.get(field,"?"),
                InlineKeyboardMarkup([[InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data=f"add_req_{deal_id}")]]),section="deal_card"); return
        ...
        if d=="balance_topup":
            await send_section(update,
                ...
                InlineKeyboardMarkup([
                    [InlineKeyboardButton(f"⭐️ {R(ru,'Звёзды','Stars')}",callback_data="topup_cur_stars")],
                    [InlineKeyboardButton(f"💳 {R(ru,'Карта / Телефон','Card / Phone')}",callback_data="topup_cur_rub")],
                    ...
                    [InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="menu_balance")],
        ...
        if d.startswith("topup_cur_"):
            ...
            await send_section(update,txt2,InlineKeyboardMarkup([
                [InlineKeyboardButton(f"✅ {R(ru,'Я отправил','I sent')}",callback_data=f"topup_sent_{method}")],
                [InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="balance_topup")],
            ]),section="balance"); return
        ...
        if d.startswith("topup_sent_"):
            ...
            try: await q.edit_message_reply_markup(InlineKeyboardMarkup([
                [InlineKeyboardButton(f"⏳ {R(ru,'Ожидание подтверждения...','Waiting for confirmation...')}",callback_data="noop")],
                [InlineKeyboardButton(f"🏠 {R(ru,'Главное меню','Main menu')}",callback_data="main_menu")],
            ]))
            except: pass
            return
        ...
        if d=="withdraw":
            ...
            if not any(reqs.get(f) for f in ("card","ton","stars")):
                await send_section(update,
                    ...
                    InlineKeyboardMarkup([
                        [InlineKeyboardButton(f"💳 {R(ru,'Добавить карту/телефон','Add card/phone')}",callback_data="req_edit_card")],
                        [InlineKeyboardButton("💎 TON",callback_data="req_edit_ton")],
                        [InlineKeyboardButton(f"⭐️ {R(ru,'Звёзды','Stars')}",callback_data="req_edit_stars")],
                        [InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="menu_balance")],
                    ]),section="balance"); return
        ...
        if d.startswith("withdraw_"):
            ...
            await send_section(update,
                ...
                InlineKeyboardMarkup([[InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="withdraw")]]),section="balance"); return
...
async def on_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if ud.get("req_step") in ("card","ton","stars"):
            ...
            if pending:
                ...
                if d2:
                    ...
                    if creator_role_p=="seller":
                        kb=InlineKeyboardMarkup([
                            [InlineKeyboardButton(f"✅ {R(ru,'Я оплатил','I paid')}",callback_data=f"paid_{pending}")],
                            [InlineKeyboardButton(f"✉️ {R(ru,'Написать продавцу','Write to seller')}",url=pu)],
                            [InlineKeyboardButton(f"🏠 {R(ru,'Главное меню','Main menu')}",callback_data="main_menu")],
                        ])
                    else:
                    ...
                    if seller_uid_p:
                        try:
                            ...
                            seller_kb2=InlineKeyboardMarkup([
                                [InlineKeyboardButton(f"✉️ {R(rs2,'Написать покупателю','Write to buyer')}",url=f"https://t.me/{bu}" if bu else MANAGER_URL)],
                                [InlineKeyboardButton(f"🏠 {R(rs2,'Главное меню','Main menu')}",callback_data="main_menu")],
                            ])
            ...
            await update.message.reply_text(f"{Ech} <b>{R(ru,'Реквизиты сохранены!','Requisites saved!')}</b>",parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"📋 {R(ru,'Мои реквизиты','My Requisites')}",callback_data="menu_req")]])); return
        ...
        if ud.get("withdraw_step")=="req":
            ...
            await update.message.reply_text(
                ...
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton(f"✉️ {R(ru,'Менеджер','Manager')}",url=MANAGER_URL)],
                    [InlineKeyboardButton(f"🏠 {R(ru,'Главное меню','Main menu')}",callback_data="main_menu")],
                ])); return
        ...
        if step=="partner":
            ...
            if dtype=="nft":
                ...
                await send_step(f"{Eprem} <b>Telegram Premium\n\n{R(ru,'Выберите срок:','Choose period:')}</b>",
                    InlineKeyboardMarkup([[
                        InlineKeyboardButton(f"✈️ 3 {R(ru,'мес.','mo')}",callback_data="prm_3"),
                        InlineKeyboardButton(f"✈️ 6 {R(ru,'мес.','mo')}",callback_data="prm_6"),
                        InlineKeyboardButton(f"✈️ 12 {R(ru,'мес.','mo')}",callback_data="prm_12")]]))
            return
...
async def finalize_deal(update, context):
    try:
        ...
        kb=InlineKeyboardMarkup([[InlineKeyboardButton(f"🏠 {R(ru,'Главное меню','Main menu')}",callback_data="main_menu")]])
        await send_new(update,text,kb,section="deal_card")
            ...
            if puid:
                try:
                    ...
                    kb2=InlineKeyboardMarkup([
                        [InlineKeyboardButton(f"✅ {R(pr,'Присоединиться','Join')}",url=join_link)],
                        [InlineKeyboardButton(f"🏠 {R(pr,'Главное меню','Main menu')}",callback_data="main_menu")]
                    ])
    ...
    try:
        ...
        try:
            await q.edit_message_reply_markup(InlineKeyboardMarkup([
                [InlineKeyboardButton(f"⏳ {R(rb,'Ожидание подтверждения...','Waiting for confirmation...')}",callback_data="noop")],
                [InlineKeyboardButton(f"🏠 {R(rb,'Главное меню','Main menu')}",callback_data="main_menu")]
            ]))
        except: pass
...
async def show_balance(update, context):
    try:
        ...
        await send_section(update,
            ...
            InlineKeyboardMarkup([
                [InlineKeyboardButton(f"💰 {R(ru,'Пополнить','Top Up')}",callback_data="balance_topup")],
                [InlineKeyboardButton(f"💸 {R(ru,'Вывод','Withdraw')}",callback_data="withdraw")],
                [InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="main_menu")],
            ]),section="balance")
...
async def show_lang(update, context):
    try:
        ...
        rows.append([InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="main_menu")])
        await send_section(update,
...
async def show_ref(update, context):
    try:
        ...
        await send_section(update,text,InlineKeyboardMarkup([[InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="main_menu")]]),section="ref")
...
async def show_req(update, context):
    try:
        ...
        rows=[]
        if card:
            rows.append([InlineKeyboardButton(f"✏️ {R(ru,'Изменить','Edit')}",callback_data="req_edit_card"),
                         InlineKeyboardButton(f"🗑 {R(ru,'Удалить','Delete')}",callback_data="req_del_card")])
        else:
            rows.append([InlineKeyboardButton(f"➕ {R(ru,'Добавить карту / телефон','Add card / phone')}",callback_data="req_edit_card")])
        if ton:
            rows.append([InlineKeyboardButton(f"✏️ {R(ru,'Изменить','Edit')}",callback_data="req_edit_ton"),
                         InlineKeyboardButton(f"🗑 {R(ru,'Удалить','Delete')}",callback_data="req_del_ton")])
        else:
            rows.append([InlineKeyboardButton(f"➕ {R(ru,'Добавить TON','Add TON')}",callback_data="req_edit_ton")])
        if stars:
            rows.append([InlineKeyboardButton(f"✏️ {R(ru,'Изменить','Edit')}",callback_data="req_edit_stars"),
                         InlineKeyboardButton(f"🗑 {R(ru,'Удалить','Delete')}",callback_data="req_del_stars")])
        else:
            rows.append([InlineKeyboardButton(f"➕ {R(ru,'Добавить Звёзды','Add Stars')}",callback_data="req_edit_stars")])
        rows.append([InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="main_menu")])
    ...
    try:
        ...
        if not deals:
            await send_section(update,
                ...
                InlineKeyboardMarkup([[InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="main_menu")]]),section="my_deals"); return
        SNAMES={
        ...
        await send_section(update,"\n".join(lines),
            InlineKeyboardMarkup([[InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="main_menu")]]),section="my_deals")
...
async def show_top(update, context):
    try:
        ...
        await send_section(update,"\n".join(lines),
            InlineKeyboardMarkup([[InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="main_menu")]]),section="top")
    ...
    try:
        ...
        if bal<=0:
            await send_section(update,
                ...
                InlineKeyboardMarkup([[InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="menu_balance")]]),section="balance"); return
        ...
        if reqs.get("stars"): rows.append([InlineKeyboardButton(f"⭐️ {R(ru,'Звёзды → ','Stars → ')}"+reqs["stars"],callback_data="withdraw_stars")])
        else: rows.append([InlineKeyboardButton(f"⭐️ {R(ru,'Звёзды','Stars')}",callback_data="withdraw_stars")])
        if reqs.get("card"): rows.append([InlineKeyboardButton(f"💳 {R(ru,'Карта → ','Card → ')}"+reqs["card"][:12]+"...",callback_data="withdraw_card")])
        else: rows.append([InlineKeyboardButton(f"💳 {R(ru,'Карта / Телефон','Card / Phone')}",callback_data="withdraw_card")])
        rows.append([InlineKeyboardButton(f"◀️ {R(ru,'Назад','Back')}",callback_data="menu_balance")])
        await send_section(update,
    ...
    try:
        ...
        if d=="adm_user":
            ...
            await q.message.edit_text("<b>Введите @юзернейм или числовой ID:</b>",parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад",callback_data="adm_back")]])); return
        ...
        if d=="adm_set_extra_group":
            ...
            await q.message.edit_text(
                ...
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад",callback_data="adm_log_channel")]])); return
        ...
        if d=="adm_reviews":
            ...
            rows2.append([InlineKeyboardButton("◀️ Назад",callback_data="adm_back")])
            await q.message.edit_text("\n".join(lines),parse_mode="HTML",reply_markup=InlineKeyboardMarkup(rows2)); return
    ...
    try:
        ...
        if step=="lt_edit":
            ...
            await update.message.reply_text(f"{Ech} <b>Шаблон обновлён!</b>",parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ К шаблонам",callback_data="adm_log_templates")]]))
            ud["adm_step"]=None; return
