...
async def send_log_msg(context, db, entry):
    ...
    try:
        ...
        # ФИКСl: убраны {user}, {deal}, {extra}, {time} без f-string — теперь используем .replace()
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # ── ФИКС: cry_ton/cry_usd теперь правильно ставят step="amount" ──
        if d=="cry_ton":
            ud["currency"]="TON"; ud["step"]="amount"
            try: await q.message.delete()
            except: pass
            msg=await update.effective_chat.send_message(
                f"{ce('6039802097916974085','💎')} <b>{R(ru,'Введите сумму','Enter amount')} (TON):</b>",
                parse_mode="HTML")
            ud["last_msg"]=msg.message_id; return
        ...
        if d=="cry_usd":
            ...
            msg=await update.effective_chat.send_message(
                f"{ce('5974217466270716579','💵')} <b>{R(ru,'Введите сумму','Enter amount')} (USDT):</b>",
                parse_mode="HTML")
            ud["last_msg"]=msg.message_id; return

        # ── ФИКС: prm_3/6/12 — правильно ставят step="currency" ──
        if d in ("prm_3","prm_6","prm_12"):
            ...
            msg=await update.effective_chat.send_message(
                f"{Egm} <b>{R(ru,'Выберите валюту:','Choose currency:')}</b>",
                reply_markup=cur_kb(lang),parse_mode="HTML")
            ud["last_msg"]=msg.message_id; return

        # ── ФИКС: cur_* — правильно ставят step="amount" ──
        ...
        if d.startswith("topup_sent_"):
            ...
            # ФИКС: эмодзи ожидания через ce() вместо icon_custom_emoji_id (не поддерживается в edit)
            try: await q.edit_message_reply_markup(InlineKeyboardMarkup([
                [InlineKeyboardButton(f"{ce('5906598824012420908','⏳')} {R(ru,'Ожидание подтверждения...','Waiting for confirmation...')}",callback_data="noop")],
                [InlineKeyboardButton(R(ru,"Главное меню","Main menu"),callback_data="main_menu",icon_custom_emoji_id="5316887736823591263")],
            ]))
            except: pass
            return
...
async def on_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # ── ФИКС: topup_step обрабатывается только если НЕТ активной сделки ──
        ...
        # ── ФИКС: step="cry_currency" и step="prem_period" не обрабатывались в on_msg ──
        # Эти шаги обрабатываются через кнопки (callbacks), сюда попадать не должны
        if step in ("cry_currency","prem_period","currency"):
            # Пользователь написал текст вместо нажатия кнопки
            await update.message.reply_text(
                f"{Ewrn} <b>{R(ru,'Пожалуйста, выберите вариант из кнопок выше.','Please choose an option from the buttons above.')}</b>",
                parse_mode="HTML"); return
...
async def on_paid(update, context):
    try:
        ...
        # ФИКС: эмодзи ожидания через ce() для кнопки
        try:
            await q.edit_message_reply_markup(InlineKeyboardMarkup([
                [InlineKeyboardButton(f"{ce('5906598824012420908','⏳')} {R(rb,'Ожидание подтверждения...','Waiting for confirmation...')}",callback_data="noop")],
