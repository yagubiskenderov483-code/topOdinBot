...
async def resume_after_requisite_saved(update, context, uid, lang, u=None, notify=True):
    """Continue deal/join/profile flow after a requisite was saved."""
    ...
    if ud.pop("req_after_buyer_deal",None):
        ...
        if resume=="amount" or (ud.get("currency") and ud.get("type") and ud.get("partner")):
            ...
            msg=await chat.send_message(deal_amount_prompt(cur,lang),parse_mode="HTML")
    ...
    if pending:
        ...
        if not user_has_requisites_for(u, deal_cur):
            ...
            set_req_input_state(
                uid, requisite_field_for_currency(deal_cur),
                mode="join", deal_id=pending, after_buyer=False)
            await chat.send_message(
...
def stash_currency_for_req(ud, currency, uid=None):
    ...
    if uid:
        field=requisite_field_for_currency(currency)
        set_req_input_state(
            uid, field, mode="deal_create", after_buyer=True, req_resume="amount")
...
def parse_nft_links_input(text):
    import re
    parts=re.split(r"[\n,;]+", (text or "").strip())
    return [p.strip() for p in parts if p.strip()]

def parse_and_validate_nft_links(text, dtype):
    parts=parse_nft_links_input(text)
    if not parts: return False,"empty",[]
    links=[]
    seen=set()
    for part in parts:
        clean=normalize_nft_link(part)
        ok,err=validate_nft_link(clean,dtype)
...
AI_KB = {
    "deal": {
        ...
        "ru": (
            ...
            "5) Для NFT - одна или несколько ссылок (каждая с новой строки); для Username - t.me/… или @username; для Stars - количество; для Premium - срок.\n"
        ...
        "en": (
            ...
            "5) NFT needs one or more links (one per line); Username needs t.me/… or @username; Stars need count; Premium needs period.\n"
    ...
    "types": {
        ...
        "ru": (
            "Типы сделок\n\n"
            "• NFT подарок - сделка по NFT-подарку (можно указать несколько ссылок, каждая с новой строки).\n"
        ...
        "en": (
            "Deal types\n\n"
            "• NFT Gift - NFT gift deal (one or more gift links, one per line).\n"
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        bindton_param=args and args[0].lower().startswith("bindton_")
        save_db(db)
        if not bindton_param:
        ...
        if bindton_param:
            ...
            if not addr:
                await update.effective_message.reply_text(
                    ...
                    parse_mode="HTML")
                restore_req_input_state(context.user_data, uid)
                await show_req(update,context); return
            db=load_db()
            u=get_user(db,uid)
            ...
            await update.effective_message.reply_text(
                ...
                parse_mode="HTML")
            u=get_user(load_db(),uid)
            await resume_after_requisite_saved(update, context, uid, lang, u, notify=False); return
        ...
        if args and args[0].lower().startswith("deal_"):
            ...
            if not user_has_requisites_for(u, deal_cur):
                ...
                set_req_input_state(
                    uid, requisite_field_for_currency(deal_cur),
                    mode="join", deal_id=deal_id, after_buyer=False)
                await send_new(
                    update,
                    f"{Ewrn} <b>{req_add_for_amount_text(deal_cur, lang, join=True)}</b>",
...
async def on_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if ud.get("req_step") in REQ_FIELDS:
            ...
            for k in ("card_step","card_pending","card_bank_name"): ud.pop(k,None)
            await resume_after_requisite_saved(update, context, uid, lang, u); return
        ...
        if step=="partner":
            ...
            if dtype=="nft":
                ...
                await send_step(
                    ...
                    f"<i>{L(lang,'Можно несколько — каждая с новой строки','Multiple links allowed — one per line','Можна кілька — кожна з нового рядка')}</i>")
            elif dtype=="username":
                ud["step"]="trade_usr"
        ...
        if step=="nft_link":
            ...
            if not ok:
                await update.message.reply_text(
                    f"{Ewrn} <b>{L(lang,'Некорректная ссылка. Формат: t.me/nft/НазваниеНФТ (можно несколько — каждая с новой строки)','Invalid link. Format: t.me/nft/NFTName (multiple links — one per line)')}</b>",
                    parse_mode="HTML"); return
            ud["nft_links"]=links
...
async def finalize_deal(update, context):
    try:
        ...
        if not user_has_requisites_for(u_check, currency):
            ...
            stash_currency_for_req(ud, currency, user.id)
            await update.effective_chat.send_message(
