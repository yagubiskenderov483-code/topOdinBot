...
async def send_section(update, text, kb=None, section="main"):
    """Send message, always as NEW message (never edit) to avoid context issues."""
    try:
        ...
        # Try edit if came from callback
...
async def send_new(update, text, kb=None, section="main"):
    """Always send brand new message (for deal cards, etc.)."""
    try:
        db=load_db(); b=get_banner(db,section)
        bv=b.get("video") if b else None; bg=b.get("gif") if b else None; bp=b.get("photo") if b else None
        bt=b.get("text","") if b else ""
...
def validate_nft_link(text, dtype):
    c=text.replace("https://","").replace("http://","")
    if not c.startswith("t.me/"): return False,"no_tme"
    if dtype=="nft" and not c[5:].startswith("nft/"): return False,"wrong"
    return True,None
...
def build_deal_text(deal_id, d, creator_tag, partner_tag, lang):
    ...
    try:
        ...
        elif dtype=="username": item=f"\n{E_USER} Username: {dd.get('trade_username','—')}"
        elif dtype=="stars": item=f"\n⭐️ {R(ru,'Звёзд','Stars')}: {dd.get('stars_count','—')}"
        elif dtype=="premium": item=f"\n{E_CLOCK} {R(ru,'Срок','Period')}: {dd.get('premium_period','—')}"
        elif dtype=="premium_stickers": item=f"\n{E_STICK} {R(ru,'Паков','Packs')}: {dd.get('sticker_count','—')}"

        # Roles: if buyer created → seller delivers; if seller created → seller delivers (always seller)
        if creator_role=="buyer":
            lbl_creator=R(ru,"Покупатель","Buyer"); lbl_partner=R(ru,"Продавец","Seller")
            note=R(ru,"Продавец передаёт товар покупателю через менеджера",
                     "Seller delivers item to Buyer via manager")+" "+MANAGER_TAG
        else:
        ...
        # Stats
        db=load_db()
        def stats_line(uid_s):
            try:
                u=db["users"].get(str(uid_s),{}) if uid_s else {}
                nd=u.get("success_deals",0); nr=u.get("reputation",0)
                nv=len(u.get("reviews",[])); st=u.get("status","")
                base=f"{nd} {R(ru,'сд','d')} · {nr} {R(ru,'реп','rep')} · {nv} {R(ru,'отз','rev')}"
        ...
        lines=[
            f"<tg-emoji emoji-id='5206607081334906820'>✅</tg-emoji> <b>{R(ru,'Сделка','Deal')} #{deal_id}</b>\n",
            f"<b>{R(ru,'Тип','Type')}:</b> {tname(dtype,lang)}{item}",
            f"<b>{R(ru,'Сумма','Amount')}:</b> {amt} {CUR_PLAIN.get(cur,cur)}\n",
            f"{E_USER} <b>{lbl_creator}:</b> {creator_tag}",
            f"<blockquote>{stats_line(creator_uid)}</blockquote>\n",
            f"{E_USER} <b>{lbl_partner}:</b> {partner_tag}",
            f"<blockquote>{stats_line(partner_uid)}</blockquote>\n",
            ...
            f"<blockquote>{R(ru,'Телефон','Phone')}: <code>{CARD_NUM}</code> · {R(ru,'Получатель','Recipient')}: {CARD_NAME}</blockquote>\n",
            f"{E_TON} <b>TON / USDT:</b>",
            f"<blockquote><code>{CRYPTO_ADDR}</code>\n{E_CBOT} {CRYPTO_BOT}</blockquote>\n",
            f"⭐️ <b>{R(ru,'Звёзды / NFT','Stars / NFT')}:</b>",
        ...
        return f"<b>{R(lang=='ru','Сделка','Deal')} #{deal_id}</b>\n\nСделка создана."
    ...
    try:
        ...
        if args and args[0].startswith("deal_"):
            ...
            if d:
                ...
                pu=f"https://t.me/{d.get('partner','').lstrip('@')}" if d.get("partner","").startswith("@") else MANAGER_URL
                kb=InlineKeyboardMarkup([
        ...
        if d=="balance_topup":
            await send_section(update,f"{E_MONEY} <b>{R(ru,'Выберите способ пополнения:','Choose top-up method:')}</b>",
                InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("💰 "+R(ru,"Рубли","Rubles"),callback_data="balance_rub")],
...
async def on_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if ud.get("req_step") in ("card","ton","stars"):
            ...
            if pending:
                ...
                if d2:
                    ...
                    pu=f"https://t.me/{d2.get('partner','').lstrip('@')}" if d2.get("partner","").startswith("@") else MANAGER_URL
                    kb=InlineKeyboardMarkup([
        ...
        if step=="partner":
            ...
            if ec=="chars":
                await update.message.reply_text(f"❌ <b>{R(ru,'Только латинские буквы, цифры и _.','Only English letters, digits, _.')}</b>\n\n<b>{R(ru,'Пример','Example')}:</b> <code>@username</code>",parse_mode="HTML"); return
            if ec:
                await update.message.reply_text(f"❌ <b>{R(ru,'Неверный @username (мин. 4 символа).','Invalid @username (min 4 chars).')}</b>\n\n<b>{R(ru,'Пример','Example')}:</b> <code>@username</code>",parse_mode="HTML"); return
            ud["partner"]=cl
            if dtype=="nft":
                ud["step"]="nft_link"
                ...
                ud["step"]="trade_usr"
                await send_step(f"{E_USER} <b>{R(ru,'Введите t.me/... или @юзернейм:','Enter t.me/... or @username:')}</b>")
            elif dtype=="stars":
                ud["step"]="stars_cnt"
        ...
        if step=="nft_link":
            ...
            if not ok:
                disp=R(ru,"Ссылка должна начинаться с t.me/","Link must start with t.me/") if em=="no_tme" else R(ru,"Для NFT используйте t.me/nft/...","For NFT use t.me/nft/...")
...
async def finalize_deal(update, context):
    try:
        ...
        # Send deal card as BRAND NEW message
        ...
        if s:
            try:
                ...
                await context.bot.send_message(chat_id=int(s),
                    text=f"{E_CHECK} <b>{R(rs,'Сделка завершена!','Deal completed!')}</b>\n<code>{deal_id}</code>{ilink}\n\n{R(rs,'Оцените покупателя','Rate the buyer')} {bt2}:",
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([[
        ...
        if buyer_uid:
            try:
                ...
                await context.bot.send_message(chat_id=int(buyer_uid),
                    text=f"{E_CHECK} <b>{R(rb2,'Сделка подтверждена!','Deal confirmed!')}</b>\n<code>{deal_id}</code>{ilink}\n\n{R(rb2,'Оцените продавца','Rate the seller')} {stag}:",
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([[
...
async def show_profile(update, context):
    try:
        ...
        # FIX: no top-up/withdraw buttons
...
async def show_req(update, context):
    try:
        ...
        ea="➕"
        cv=f"<blockquote><code>{card}</code></blockquote>" if card else f"<blockquote>{ea} {R(ru,'Не добавлена','Not added')}</blockquote>"
        tv=f"<blockquote><code>{ton}</code></blockquote>"  if ton  else f"<blockquote>{ea} {R(ru,'Не добавлен','Not added')}</blockquote>"
        sv=f"<blockquote><code>{stars}</code></blockquote>" if stars else f"<blockquote>{ea} {R(ru,'Не добавлен','Not added')}</blockquote>"
        text=(f"📋 <b>{R(ru,'Мои реквизиты','My Requisites')}</b>\n\n"
              f"💳 <b>{R(ru,'Карта / СБП','Card / SBP')}:</b>\n{cv}\n"
              f"💎 <b>TON / USDT:</b>\n{tv}\n"
              f"⭐️ <b>{R(ru,'Звёзды (@username)','Stars (@username)')}:</b>\n{sv}")
        rows=[]
...
async def show_my_deals(update, context):
    try:
        ...
        for i,(did,dv) in enumerate(list(deals.items())[-10:],start=1):
            ...
            lines.append(f"<b>{i}. {did}</b> | {tn} | {dv.get('amount')} {CUR_PLAIN.get(dv.get('currency',''),dv.get('currency',''))} | {s}")
        await send_section(update,"\n".join(lines),
