            ...
            if is_creator:
                if viewer_role=="buyer":
                    if d.get("item_transferred"):
                        joined_instr=R(ru,
                            f"Продавец передал товар. Переведите <b>{show_amt} {cur_plain(show_cur,lang)}</b> по реквизитам ниже и нажмите «Я оплатил».",
                            f"The seller transferred the item. Transfer <b>{show_amt} {cur_plain(show_cur,lang)}</b> using the details below and press «I paid».")
                    else:
                        joined_instr=R(ru,
        ...
        if args and args[0].startswith("deal_"):
            ...
            if d:
                ...
                if not user_has_requisites(u):
                    ...
                    await send_new(
                        update,
                        f"{Ewrn} <b>{R(ru,'Чтобы присоединиться к сделке, сначала добавьте реквизиты.','To join the deal, add requisites first.')}</b>",
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if d.startswith("prem_cur_"):
            ...
            ud["currency"]=pc_map.get(pc,pc.upper()); ud["pay_currency"]=ud["currency"]; ud["step"]="amount"
            try: await q.message.delete()
            except: pass
            msg=await update.effective_chat.send_message(
        ...
        if d.startswith("cur_"):
            ud["currency"]=CURMAP.get(d,d); ud["pay_currency"]=ud["currency"]; ud["step"]="amount"
    ...
    try:
        ...
        if card:
            rows.append([InlineKeyboardButton(R(ru,"Изменить карту","Edit card"),callback_data="req_edit_card"),
                         InlineKeyboardButton(R(ru,"Удалить карту","Delete card"),callback_data="req_del_card")])
        else:
        ...
        if ton:
            rows.append([InlineKeyboardButton(R(ru,"Изменить TON","Edit TON"),callback_data="req_edit_ton"),
                         InlineKeyboardButton(R(ru,"Удалить TON","Delete TON"),callback_data="req_del_ton")])
        else:
            rows.append([InlineKeyboardButton(R(ru,"Добавить TON","Add TON"),callback_data="req_edit_ton")])
        if stars:
            rows.append([InlineKeyboardButton(R(ru,"Изменить Звёзды","Edit Stars"),callback_data="req_edit_stars"),
                         InlineKeyboardButton(R(ru,"Удалить Звёзды","Delete Stars"),callback_data="req_del_stars")])
        else:
